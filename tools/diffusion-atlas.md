...
Alpha, v0.3.12. M1 (app shell + hosted generation), M2 (local FastAPI + Denoise Trajectory), M3 (Guidance Sweep, Latent Neighbourhood), and M4 (Compositional Bench with CLIP scoring) are all shipped. Recent work hardens the production path: scheduler swap from DPM++ 2M Karras to DPM++ 2M after a late-trajectory underflow, NaN-in-VAE detection, empty-prompt defences, and IDB resilience (`withDB()` retry-on-close so HMR / version-change / quota races do not kill cache writes). Up next on the 0.3.x track: Clippy / Hackerman easter eggs, object-detection-based Bench scoring (proper GenEval rather than CLIP cosine), Library run loader (replay saved comparison runs), attention-map visualisation, and h-space steering.
